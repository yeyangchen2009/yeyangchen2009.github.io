...
不用翻别的文章，**你现在滚到这篇文章评论按钮的上方**，就能看到两块卡片：左边通向上一篇 [G07 写作三件套](/post/13.html)，右边是一块灰色的"已经到尽头啦"——因为本文发布时它就是最新的一篇。这个导航在全站每篇文章底部都有，颜色自动跟随明暗主题，手机上同样保持左右两栏，只是更紧凑（标签带 ← → 箭头、日期让位隐藏）。
...
**为什么手机端坚持横排。** 初版在 600px 以下把两张卡片改成纵向堆叠，真机上看反而别扭：竖屏本来就长，导航再占两大块纵向空间，"上一篇/下一篇"的并列对照感也丢了。现在的做法是小屏**保持左右两栏**，只做减法：卡片间距和内边距收紧、标题降到 13px、日期隐藏（移动端时间信息价值最低）、标签用伪元素补上 `←` `→` 箭头，方向感一眼可辨。375px 宽的屏幕上每栏还有约 170px，两行标题足够表达。
...
(function () {
    ...
    function injectStyle() {
        ...
        style.textContent =
          ...
          + '.gmeek-pn-title{font-size:13px;line-height:1.35;}'
