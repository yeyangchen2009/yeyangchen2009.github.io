...
上一篇 [《用 GitHub Issues 写博客》](/post/1.html) 讲完整个搭建过程后，我自己回头查资料时先遇到的问题就是：文章一长，想跳到某个小节只能 Ctrl+F。成熟博客一般都会在正文旁侧放一个 **TOC（Table of Contents，文章目录）**，作用有三个：

1. **导航**：点击标题直接跳转到对应章节；
2. **定位**：滚动时知道自己处在文章的什么位置；
3. **预览**：点开文章先扫一遍目录，就知道这篇值不值得读完。

Gmeek 官方文档站（blog.meekdai.com）的文章页右侧就有一个，我打开开发者工具扒了一遍，发现它正是 Gmeek 源码仓库 `plugins/` 目录里自带的插件——`GmeekTOC.js`。整个插件只有 100 行原生 JavaScript，零依赖。
...
我选择了前者。本博客所有静态资源都坚持放在自己仓库里（连 Mermaid 都是本地化的），不把可用性押在第三方 CDN 上。
...
下一篇准备系统盘点一下：一个博客除了评论和互动，还需要哪些基础功能，以及我给这个博客定下的建设路线图。
