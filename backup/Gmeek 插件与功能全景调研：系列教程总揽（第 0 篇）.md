...
| 候选 | 内容 | 依据 | 状态 |
| --- | --- | --- | --- |
| **G17 SEO 收尾战** | 文章页补 `<link rel="canonical">`、JSON-LD（BlogPosting 结构化数据）、twitter:card；顺手清洁 `meta description`——框架当前直接取正文开头，会把 `>` 引用符和 `[文字](链接)` 语法原样塞进搜索摘要 | #4 清单上仅剩的技术缺口（已 curl 实锤框架不自带） | 🎯 首选，随时可写 |
