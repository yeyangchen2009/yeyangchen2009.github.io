...
| B01 | [18 秒建站实录：从一个 GitHub 账号到第一篇文章上线](/post/32.html) | 零起点动手篇：注册→模板建仓→Pages 开关→建标签发 issue→看绿勾；user/project site 命名规则、三考点、翻车急诊室 | 地基篇 | ✅ |
| B02 | [毛坯房装修：认识 config.json，站名、头像换成你自己的](/post/34.html) | 四必填字段、头像 URL 技巧与 favicon/ogImage 继承、JSON 三戒、runOne 旧快照覆盖源码机制、last 与锁版本 | 地基篇 | ✅ |
| B03 | [在 Issues 里过日子：写稿、改稿、置顶与下架的编辑部日常](/post/35.html) | Preview 与博客同用 GitHub `/markdown` GFM 引擎、无标签 315 行门与 #22 KeyError 活例、labels[0] 主标签、edited 自动重建但 createdAt 不变、yml 事件表实锤（pin/close/标签/config 全手动 runAll）、pinned→top=1 按 (top,createdAt) 排序、Close+runAll 两步下架与 Reopen、末行 timestamp 补发旧文 | 地基篇 | ✅ |

更新方式：每篇教程发布后，叶扬会回来编辑本文（Gmeek 监听 issue 的 `edited` 事件，编辑即自动重建），所以这张表会一直是最新的。

## 地基篇（B 系列）：从零开始的八篇规划（2026-09-16 启动）

G 系列默认读者**已经有一个博客**。不少新读者反馈"连仓库都还没建，不知道该从哪篇看起"，于是另开一条零起点线：[B00 地基篇总览](/post/30.html)已发布，讲清 Issues/Actions/Pages 三块基石、引擎/模板/自己三个仓库的关系、一次构建的完整流水线与 `runAll`/`runOne` 分支。后续八篇每篇一个主题，不跳步：

| 编号 | 主题 | 核心内容 | 状态 |
|---|---|---|---|
| B00 | [地基篇总览](/post/30.html) | 三基石、三仓库、构建流水线、引擎导游 | ✅ |
| B01 | [18 秒建站实录](/post/32.html) | 注册到首篇带标签文章上线；命名、Pages 开关、标签开关三考点 | ✅ |
| B02 | [毛坯房装修：认识 config.json](/post/34.html) | 四必填字段、头像 URL 与继承、JSON 三戒、旧快照覆盖源码机制、last 与锁版本 | ✅ |
| B03 | [在 Issues 里过日子](/post/35.html) | GFM 同引擎、无标签不收、edited 自动重建但日期不变、事件表（pin/close/标签/config 全手动）、Pin 排序、Close 下架与 Reopen、timestamp 补发旧文 | ✅ |
| B04 | 评论与关于页 | utterances app 安装；singlePage 做 About 固定页 | ⏳ |
| B05 | Actions 篇 | 工作流 YAML 逐行、三种触发、红叉日志、自动提交是什么 | ⏳ |
| B06 | Pages 篇 | artifact 部署 vs 分支部署、URL 推导、HTTPS/CDN/缓存、自定义域名 | ⏳ |
| B07 | 备份、搬家与升级 | backup 离线副本、blogBase.json、GMEEK_VERSION 升级、换账号迁移 | ⏳ |
| B08 | 轻装修与进阶指路 | 三态主题、页脚、favicon、RSS；收尾后指路 G 系列 | ⏳ |

地基篇与进阶篇的分工：**B 系列让你拥有一个博客，G 系列让你拥有"你的"博客。**
...
G01–G16 正篇与 **[G17 SEO 收尾战](/post/27.html)**（canonical + JSON-LD + twitter:card + 清洁 description，前传 #4 清单清零）均已完成；方法论番外已有四篇：**[番外·给博客拍证件照](/post/26.html)**（零依赖 CDP 无头截图，成品脚本 `tools/cdp-shot.js`）、**[番外二·开源回馈流水线](/post/28.html)**（三个上游 PR 待合并，见上方 2026-09-16 小节）、**[番外三·gdigrab 抓窗口](/post/31.html)**（浏览器外截图，成品脚本 `tools/shot-window.ps1`）与 **[番外四·点到为止](/post/33.html)**（借用户已登录浏览器窗口拍登录态页面，Win32 UI 自动化 + 安全边界，成品脚本 `tools/ui-shot.ps1`；截图三部曲——浏览器内 CDP／浏览器外 gdigrab／登录态借窗——至此收束）。2026-09-16 另发布 **[G18 被浮动挤扁的"上一篇/下一篇"](/post/29.html)**：G08 导航卡片在移动端被版权小字的 `float:right` + BFC 避让规则压成 60% 窄列、标题竖排；`clear:both` 拿回全宽、小屏去浮动、meta 行合并，移动/桌面同取景前后对比验收。2026-09-16 另开零起点地基篇，**[B00 总览](/post/30.html)**、**[番外三](/post/31.html)**（截图工具箱扩到浏览器外，成品 `tools/shot-window.ps1`）相继发布；2026-09-17 发布地基篇第一篇动手文 **[B01 18 秒建站实录](/post/32.html)**（注册→模板建仓→Pages 开关→标签发文→绿勾验收全流程，含时序图与翻车急诊室）；同日补齐 4 张登录态实拍图——不碰凭据、不给日常 Edge 开调试端口，改用 Win32 UI 自动化（EnumWindows 按用户名正向定位窗口、F11 全屏、`msedge.exe <url>` 命令行委派新标签导航、剪贴板灌中文标题、mouse_event 点开下拉、gdigrab 裁剪），占位图同名替换零改正文；同日发布 **[番外四·点到为止](/post/33.html)**（把 B01 补拍过程写成独立一篇：Win32 UI 自动化借用户已登录 Edge 拍登录态页的六场翻车——F11 按错窗口/SendKeys 静默失灵/任务栏自动隐藏骗全屏/工具栏滑出条/盲点跳 Docs/黑帧，灵魂章是"点到为止"安全边界——可点可逆 vs 绝不碰提交按钮、还原现场、逐张隐私检查；成品通用脚本 `tools/ui-shot.ps1`，已沉淀成本机 screenshot 技能"路径 C"）；同日发布地基篇第二篇动手文 **[B02 毛坯房装修：认识 config.json](/post/34.html)**（title/subTitle/avatarUrl/GMEEK_VERSION 四必填字段逐个拆，`https://github.com/用户名.png` 头像技巧与 favicon/ogImage 默认继承，JSON 三戒配 `python -m json.tool`/`jq` 自检实拍；核心章翻开源码讲透"改完配置为何不生效"——runOne 启动时用 docs/blogBase.json 旧快照逐项覆盖新配置、且 push config 不在工作流触发事件里，故必须手动 runAll；cleanFile 只在 runAll 复制 static 的第二定律一并补齐；"last"=构建时刻最新 tag 的浮动机制与锁版止血；含 runAll/runOne 分支暗色 mermaid 流程图。配图两张走路径 C 借窗实拍文件页/编辑器页只拍不提交，tags 页 CDP，json 校验走 shot-window 拍经典控制台；期间给 `tools/cdp-shot.js` 加了 `--proxy` 选项）；同日发布地基篇第三篇动手文 **[B03 在 Issues 里过日子](/post/35.html)**（编辑部全套日常：New issue 三件套与"Preview 即所得"的硬证据——构建走 GitHub 官方 `/markdown` GFM 接口与编辑器同引擎；翻源码讲透无标签 issue 在 [Gmeek.py:315](https://github.com/Meekdai/Gmeek/blob/main/Gmeek.py#L315) 被跳过且编号取数 KeyError，#22 是现成活例；改稿监听 `edited` 自动重建但 createdAt 只认创建时间、编号制 URL 不随标题变；核心章用模板 yml 触发名单+暗色 mermaid 给"哪些动作自动开工"列事件表——只有 opened/edited 自动，Pin/Close/标签/config/static 全手动 runAll；Pin 原理是事件时间线 `pinned`→top=1、按 (top,createdAt) 排序只动序不动日；Close 后旧页要等 runAll 清空重生成才 404、Reopen 是后悔药；末行秒级 timestamp 补发旧文。配图七张：Write/Preview/Pin 菜单/Close 四张走路径 C 借窗只拍不按，tag 胶囊页与首页置顶实拍走 CDP——为拍真实置顶效果，经作者同意用 GraphQL `pinIssue` 把本总揽 #5 **长期置顶**（REST pin 端点对该 token 404，GraphQL 成功；随时可 Unpin 还原）；Actions 运行列表那张拍摄期遇 github.com 网页通道间歇中断，先上同名占位卡、当天通道恢复后已用真图（Issue opened / Issue edited / Manually run 三类同屏全绿；Scheduled 每日北京 0 点仅一条、无法与白天运行同屏，正文如实说明）替换并手动 runAll，正文零改动；期间给 `tools/ui-shot.ps1` 加了 `-PasteFile`（UTF-8 多行正文灌剪贴板）、`-ClearField`（Ctrl+A/Delete 清框）与无 -Out 的 action-only 模式，并把操作顺序重构为点击聚焦→清空→粘贴）；下一篇动手向为 **B04 评论与关于页**（utterances 评论区 + singlePage 做 About 固定页）；G 系列方向下一篇待写仍是约 2026-09-21 的 **G04 收录实战番外**——用 Google/Bing 的真实收录数据回答"RSS 当 sitemap 提交到底有没有用"，届时一并验收 G17 富结果与清洁摘要的真实生效情况、追踪上游 PR 进展。更远的候选见上方选题池。
